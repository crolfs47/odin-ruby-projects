...
def stock_picker(stock_prices)
  ...
  max_buy_index = 0
  max_sell_index = 0
  stock_prices.each_with_index do |_buy_stock, buy_index|
    stock_prices[buy_index..-1].each_with_index do |_sell_stock, sell_index|
...
def stock_pickerx(stock_prices)
  ...
  stock_prices.each_with_index do |_stock, index|
    ...
    sell_price = if index == num_stocks - 1
                   buy_price
                 else
                   stock_prices.last(num_stocks - index - 1).max
                 end
    profit = sell_price - buy_price
